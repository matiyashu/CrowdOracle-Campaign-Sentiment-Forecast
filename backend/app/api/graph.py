...
Uses a server-side project context to persist state between requests.
"""
...
# Logger
logger = get_logger('bigbrother.api')
...
def reset_project(project_id: str):
    """
    Reset the project status (for rebuilding the graph).
    """
    project = ProjectManager.get_project(project_id)
...
def generate_ontology():
    """
    Endpoint 1: upload files, analyze and generate an ontology definition.
...
def build_graph():
    """
    Endpoint 2: build the knowledge graph for a given project_id.
    ...
    try:
        ...
        # If forced rebuild is requested, reset state
        ...
        # Read configuration
        graph_name = data.get('graph_name', project.name or 'BigBrother Graph')
        ...
        def build_task():
            ...
            build_logger = get_logger('bigbrother.build')
            try:
