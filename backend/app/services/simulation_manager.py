...
Manages parallel Twitter and Reddit simulations using pre-built scripts
and LLM-generated configuration parameters.
"""
...
logger = get_logger('bigbrother.simulation')
...
class SimulationManager:
    ...
    2. Generate OASIS Agent Profiles for each entity.
    3. Use LLM to intelligently generate simulation configuration parameters.
    4. Prepare all files required by the pre-built simulation scripts.
    """
    ...
    def _get_simulation_dir(self, simulation_id: str) -> str:
        """Return the data directory for a simulation, creating it if needed."""
    ...
    def prepare_simulation(
        ...
        Steps:
        1. Read and filter entities from the Zep knowledge graph.
        2. Generate an OASIS Agent Profile for each entity (optionally LLM-enhanced, parallel).
        3. Use LLM to generate simulation configuration (time, activity, post frequency, etc.).
        4. Save configuration and profile files.
        5. Simulation scripts remain in backend/scripts/ and are invoked from there.

        Args:
            simulation_id: Simulation ID
            simulation_requirement: Natural-language description used for LLM config generation
            document_text: Raw document content providing background context for the LLM
            defined_entity_types: Pre-defined entity types to filter (optional)
        ...
        try:
            ...
            profiles = generator.generate_profiles_from_entities(
                ...
                graph_id=state.graph_id,  # Enables Zep retrieval for richer context
            ...
            # Reddit was already saved in real-time during generation; this saves again for completeness.
            if progress_callback:
                progress_callback(
            ...
            if state.enable_twitter:
                # Twitter must use CSV format — OASIS requirement
                generator.save_profiles(
                    profiles=profiles,
            ...
            # Note: run scripts stay in backend/scripts/; simulation_runner invokes them from there.
    ...
    def get_simulation_config(self, simulation_id: str) -> Optional[Dict[str, Any]]:
        """Return the simulation configuration dict, or None if not yet generated."""
        sim_dir = self._get_simulation_dir(simulation_id)
        config_path = os.path.join(sim_dir, "simulation_config.json")
    ...
    def get_run_instructions(self, simulation_id: str) -> Dict[str, str]:
        """Return shell commands and paths needed to manually run the simulation."""
        sim_dir = self._get_simulation_dir(simulation_id)
        config_path = os.path.join(sim_dir, "simulation_config.json")
        ...
        return {
            ...
            "instructions": (
                f"1. Activate conda environment: conda activate BigBrother\n"
