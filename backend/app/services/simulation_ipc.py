...
via a simple filesystem-based command/response pattern:
1. Flask writes commands to the commands/ directory.
2. The simulation script polls the directory, executes commands, and writes
...
logger = get_logger('bigbrother.simulation_ipc')
...
class SimulationIPCClient:
    ...
    def send_command(
        ...
        Raises:
            TimeoutError: Response did not arrive within the timeout
        """
    ...
    def send_interview(
        ...
        Args:
            ...
            platform: Target platform (optional)
                ...
                - None: Interview on both platforms (dual-platform sim) or whichever is active
            timeout: Timeout in seconds
    ...
    def send_batch_interview(
        ...
        Args:
            interviews: List of interview items, each {"agent_id": int, "prompt": str, "platform": str (optional)}
            platform: Default platform (optional, overridden per item)
                - "twitter": Default to Twitter only
                - "reddit": Default to Reddit only
                - None: Both platforms in dual-platform sim
            timeout: Timeout in seconds
    ...
    def check_env_alive(self) -> bool:
        """
        Check whether the simulation environment is still running.

        Reads the env_status.json file written by the IPC server.
        """
...
class SimulationIPCServer:
    ...
    def poll_commands(self) -> Optional[IPCCommand]:
        """
        Poll the commands directory and return the oldest pending command.
        ...
        # Sort command files by modification time (oldest first)
        command_files = []
    ...
    def send_response(self, response: IPCResponse):
        ...
        # Delete the corresponding command file
