...
excessively long content in one shot:
...
logger = get_logger('bigbrother.simulation_config')
...
CHINA_TIMEZONE_CONFIG = {
    ...
    "activity_multipliers": {
        "dead": 0.05,      # Almost no activity in the early hours
...
class AgentActivityConfig:
    ...
    # Post frequency (expected posts per simulated hour)
    ...
    # Influence weight (determines probability that this agent's posts are seen by others)
...
class TimeSimulationConfig:
    ...
    # Time represented by each round (simulated minutes) — default 60 min (1 hour) to speed up time flow
    ...
    # Peak hours (19:00–22:00 evening, most active period)
...
class EventConfig:
    ...
    # Scheduled events (triggered at specific simulated times)
...
class PlatformConfig:
    ...
    popularity_weight: float = 0.3   # Popularity / engagement
    relevance_weight: float = 0.3    # Relevance

    # Viral spread threshold (number of interactions before content spreads virally)
    viral_threshold: int = 10

    # Echo-chamber effect strength (degree to which similar viewpoints cluster together)
...
class SimulationConfigGenerator:
    ...
    Uses an LLM to analyse simulation requirements, document content, and
    graph entity information, then automatically generates optimal simulation
    parameter configurations.

    Step-by-step generation strategy:
    1. Generate time and event configurations (lightweight).
    2. Generate Agent configurations in batches of 10–20.
    ...
    def _build_context(
        ...
        remaining_length = self.MAX_CONTEXT_LENGTH - current_length - 500  # Reserve 500-character buffer
    ...
    def _try_fix_config_json(self, content: str) -> Optional[Dict[str, Any]]:
        """Attempt to repair a configuration JSON string."""
        import re
...
### General principles (reference only — adjust flexibly based on the specific event and participant group):
- Infer the target user group's timezone and daily-routine patterns from the simulation scenario; the values below are reference examples for UTC+8
- 0–5 AM: almost no activity (activity coefficient 0.05)
- 6–8 AM: gradually becoming active (activity coefficient 0.4)
- 9–18 working hours: moderate activity (activity coefficient 0.7)
- 19–22 evening peak: highest activity (activity coefficient 1.5)
- After 23:00: activity drops (activity coefficient 0.5)
- General pattern: low in the early hours, rising in the morning, moderate during work hours, peak in the evening
- **Important**: the values above are examples only — adjust specific time slots based on the nature of the event and the characteristics of the participant group
  - e.g. a student group's peak may be 21–23; media is active all day; official institutions only active during work hours
  - e.g. a breaking-news event may drive late-night discussion — off_peak_hours can be shortened accordingly
...
- reasoning (string): brief explanation of why this configuration was chosen"""
        ...
        system_prompt = "You are a social-media simulation expert. Return pure JSON. The time configuration must match the daily-routine patterns of the target user group in the simulation scenario."
    ...
    def _get_default_time_config(self, num_entities: int) -> Dict[str, Any]:
        """Get the default time configuration (Chinese daily-routine pattern)."""
        return {
            "total_simulation_hours": 72,
    ...
    def _parse_time_config(self, result: Dict[str, Any], num_entities: int) -> TimeSimulationConfig:
        """Parse the time configuration result and validate that agents_per_hour does not exceed total agent count."""
        # Read raw values
        ...
        return TimeSimulationConfig(
            ...
            off_peak_activity_multiplier=0.05,  # Almost no activity in the early hours
    ...
    def _generate_event_config(
        ...
        # Collect available entity types for the LLM to reference
...
- Design initial post content — **every post must specify a poster_type**

**Important**: poster_type must be chosen from the "Available entity types" above so that each initial post is assigned to the right Agent.
...
{{
    ...
    def _assign_initial_post_agents(
        ...
        """
        Assign an appropriate poster Agent to each initial post.
        ...
        # Type alias table (handles different formats that the LLM may output)
        type_aliases = {
        ...
        for post in event_config.initial_posts:
            ...
            # 3. If still not found, fall back to the highest-influence agent
...
- **Times should match the target user group's daily routine** — the values below are reference examples; adjust to fit the simulation scenario
...
{{
    "agent_configs": [
        {{
            ...
            "active_hours": [<list of active hours based on the user group's daily routine>],
        ...
        system_prompt = "You are a social-media behaviour analysis expert. Return pure JSON. Configurations must match the daily-routine patterns of the target user group in the simulation scenario."
    ...
    def _generate_agent_config_by_rule(self, entity: EntityNode) -> Dict[str, Any]:
        """Generate a single Agent configuration using rules (Chinese daily-routine pattern)."""
