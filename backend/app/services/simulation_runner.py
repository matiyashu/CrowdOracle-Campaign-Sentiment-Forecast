...
Runs simulations in a background process, records per-agent actions,
...
logger = get_logger('bigbrother.simulation_runner')

# Flag to track whether cleanup has been registered
...
class SimulationRunState:
    ...
    # Platform completion state (detected via simulation_end event in actions.jsonl)
...
class SimulationRunner:
    ...
    2. Parse action logs and record per-agent actions.
    ...
    _stdout_files: Dict[str, Any] = {}  # stdout file handle storage
    _stderr_files: Dict[str, Any] = {}  # stderr file handle storage
    ...
    def start_simulation(
        ...
        enable_graph_memory_update: bool = False,  # whether to update agent activities to Zep graph
        ...
        # Truncate if max_rounds was specified
        ...
        try:
            ...
            # Append max-rounds arg if specified
            if max_rounds is not None and max_rounds > 0:
            ...
            # Set working directory to the simulation directory (DB files etc. are created here).
            # start_new_session=True creates a new process group so we can kill all child processes
            # via os.killpg when the server shuts down.
            process = subprocess.Popen(
                ...
                start_new_session=True,  # new process group; killed cleanly on server shutdown
            )
    ...
    def _monitor_simulation(cls, simulation_id: str, locale: str = 'zh'):
        ...
        try:
            ...
            # Process finished — do one final log read
            if os.path.exists(twitter_actions_log):
                        ...
                        try:
                            ...
                            if "event_type" in action_data:
                                ...
                                if event_type == "simulation_end":
                                    ...
                                    # If only one platform was running, only that one is checked;
                                    # if both platforms were running, both must be done.
    ...
    def _check_all_platforms_completed(cls, state: SimulationRunState) -> bool:
        ...
        # If a platform is enabled but not yet done, return False
    ...
    def _terminate_process(cls, process: subprocess.Popen, simulation_id: str, timeout: int = 10):
        ...
        if IS_WINDOWS:
            ...
            # Unix: kill by process group.
            # Because start_new_session=True was used, pgid == main process PID.
        ...
        with open(file_path, 'r', encoding='utf-8') as f:
            for line in f:
                ...
                try:
                    ...
                    # Get platform: prefer the record's own platform field, fall back to default
    ...
    def get_all_actions(
        ...
        # Read Twitter action file (platform is auto-set to "twitter" from the file path)
        twitter_actions_log = os.path.join(sim_dir, "twitter", "actions.jsonl")
        ...
        # Read Reddit action file (platform is auto-set to "reddit" from the file path)
        reddit_actions_log = os.path.join(sim_dir, "reddit", "actions.jsonl")
        ...
        # If per-platform files don't exist, fall back to the legacy single-file format
        if not actions:
            actions_log = os.path.join(sim_dir, "actions.jsonl")
            actions = cls._read_actions_from_file(
                actions_log,
                default_platform=None,  # legacy format should already have a platform field
                platform_filter=platform,
                agent_id=agent_id,
                round_num=round_num
            )
    ...
    def cleanup_all_simulations(cls):
        ...
        # Check whether there is anything to clean (avoids spurious log output for empty processes)
        ...
        for simulation_id, process in processes:
            try:
                if process.poll() is None:  # process still running
                    ...
                    # Also update state.json to set status = stopped
                    try:
                        sim_dir = os.path.join(cls.RUN_STATE_DIR, simulation_id)
    ...
    def register_cleanup(cls):
        ...
        Call this at Flask app startup to ensure all simulation processes are
        ...
        # (the process that actually runs the app). WERKZEUG_RUN_MAIN=true indicates
        # the reloader child. Outside debug mode the variable is absent, so always register.
        is_reloader_process = os.environ.get('WERKZEUG_RUN_MAIN') == 'true'
        is_debug_mode = os.environ.get('FLASK_DEBUG') == '1' or os.environ.get('WERKZEUG_RUN_MAIN') is not None

        # In debug mode only register in the reloader child; in non-debug mode always register
        if is_debug_mode and not is_reloader_process:
            _cleanup_registered = True  # mark registered to prevent the child from trying again
            return
        ...
        def cleanup_handler(signum=None, frame=None):
            ...
            if signum == signal.SIGINT and callable(original_sigint):
                ...
                # Original handler not callable (e.g. SIG_DFL) — use default behaviour
        ...
        try:
            ...
            # Not in main thread — only atexit is available
    ...
    def interview_agents_batch(
        ...
        Args:
            ...
            platform: Default platform (optional; overridden by each item's own platform field)
    ...
    def close_simulation_env(
        ...
        try:
            ...
            # Timeout may mean the environment is already closing
            return {
                "success": True,
