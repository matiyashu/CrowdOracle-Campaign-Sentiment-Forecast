...
Features:
- After the simulation finishes, the environment is not closed immediately; it enters a
  command-waiting mode instead.
- Supports receiving Interview commands over IPC.
- Supports both single-agent interviews and batch interviews.
- Supports a remote shutdown command to terminate the environment.
...
# Load the project's root .env file (contains LLM_API_KEY and other settings)
...
# Install the filter at module load time so it takes effect before any camel code runs
logging.getLogger().addFilter(MaxTokensWarningFilter())


def setup_oasis_logging(log_dir: str):
    """Configure OASIS logging, writing to log files with fixed names."""
...
class IPCHandler:
    ...
    async def handle_interview(self, command_id: str, agent_id: int, prompt: str) -> bool:
        """
        Handle an interview command for a single agent.
        ...
        try:
            ...
            print(f"  Interview finished: agent_id={agent_id}")
            return True
    ...
    async def handle_batch_interview(self, command_id: str, interviews: List[Dict]) -> bool:
        ...
        try:
            ...
            actions = {}
            agent_prompts = {}  # Track the prompt used for each agent
            ...
            print(f"  Batch interview finished: {len(results)} agents")
            return True
    ...
    async def process_commands(self) -> bool:
        ...
        Returns:
            True to keep running, False if the runner should exit.
        """
...
class TwitterSimulationRunner:
    ...
    # Actions available on Twitter (INTERVIEW is excluded; it can only be triggered manually via ManualAction)
    AVAILABLE_ACTIONS = [
    ...
    def _get_profile_path(self) -> str:
        """Return the profile file path (OASIS Twitter uses CSV)."""
    ...
    def _create_model(self):
        ...
        Settings are taken from the project's root .env file (highest priority):
        - LLM_API_KEY: API key
        ...
        # Fall back to the config file if .env does not define a model
        if not llm_model:
    ...
    def _get_active_agents_for_round(
        ...
        """
        Decide which agents are active this round based on the time and config.

        Args:
            env: The OASIS environment.
            current_hour: The current simulated hour (0-23).
        ...
        # Baseline activation counts
        base_min = time_config.get("agents_per_hour_min", 5)
        base_max = time_config.get("agents_per_hour_max", 20)

        # Adjust by time-of-day bucket
        ...
        for cfg in agent_configs:
            ...
            # Skip agents that are not active at this hour
            if current_hour not in active_hours:
                continue

            # Decide by activity level
    ...
    async def run(self, max_rounds: int = None):
        ...
        Args:
            max_rounds: Maximum number of rounds (optional; used to cap overly long runs).
        """
        print("=" * 60)
        ...
        # Compute the total number of rounds
        total_rounds = (total_hours * 60) // minutes_per_round

        # Apply the max-rounds cap if provided
        ...
        if self.wait_for_commands:
            print("\n" + "=" * 60)
            print("Entering command-waiting mode - environment stays alive")
            print("Supported commands: interview, batch_interview, close_env")
            print("=" * 60)

            self.ipc_handler.update_status("alive")

            # Wait-for-commands loop (uses the global _shutdown_event)
            try:
...
async def main():
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help='Maximum number of simulation rounds (optional; used to cap overly long runs)'
    )
    parser.add_argument(
        '--no-wait',
        action='store_true',
        default=False,
        help='Close the environment immediately when the simulation finishes; do not wait for commands'
    )
    ...
    # Initialize logging (fixed file names, old logs are cleared)
...
def setup_signal_handlers():
    ...
    giving the program a chance to release resources (database, environment, etc.).
    """
