"""
OASIS dual-platform parallel simulation runner script
...
- Keeps environments alive after simulation completes; enters command-wait mode
- Accepts Interview commands via IPC
...
# ============================================================
# Fix Windows encoding: set UTF-8 before any imports.
# This works around OASIS third-party libraries opening files without
...
if sys.platform == 'win32':
    ...
    # This affects every open() call that does not specify an encoding.
    os.environ.setdefault('PYTHONUTF8', '1')
    os.environ.setdefault('PYTHONIOENCODING', 'utf-8')

    # Reconfigure stdout/stderr to UTF-8 (fixes garbled non-ASCII console output).
    ...
    # Note: this normally needs to be set at Python startup; setting it at
    # runtime may not take effect, so we also monkey-patch the built-in open.
    import builtins
    ...
    def _utf8_open(file, mode='r', buffering=-1, encoding=None, errors=None,
        ...
        Fixes third-party libraries (e.g. OASIS) that open files without
        specifying an encoding.
        """
        # Only override the default encoding for text mode (non-binary) when
        # no encoding was supplied.
...
# Load the project-root .env (contains LLM_API_KEY and other settings).
...
class MaxTokensWarningFilter(logging.Filter):
    """Filters out camel-ai's max_tokens warning (we intentionally omit max_tokens and let the model decide)."""
...
# Attach the filter at module-import time so it is active before camel code runs.
...
def disable_oasis_logging():
    ...
    OASIS logs every agent observation/action, which is too noisy;
    we use our own action_logger instead.
    """
    # Disable every OASIS logger.
    oasis_loggers = [
        "social.agent",
        "social.twitter",
        "social.rec",
...
def init_logging_for_simulation(simulation_dir: str):
    ...
    # Remove any stale log directory.
...
# Available Reddit actions (INTERVIEW excluded; it can only be triggered via ManualAction).
REDDIT_ACTIONS = [
    ActionType.LIKE_POST,
...
class ParallelIPCHandler:
    ...
    async def handle_interview(self, command_id: str, agent_id: int, prompt: str, platform: str = None) -> bool:
        ...
        """
        # If a platform was specified, interview only on that platform.
        ...
        # Interview both platforms concurrently.
        tasks = []
    ...
    async def handle_batch_interview(self, command_id: str, interviews: List[Dict], platform: str = None) -> bool:
        ...
        Args:
            ...
            platform: default platform (can be overridden per interview item)
                - "twitter": interview only on Twitter
                - "reddit": interview only on Reddit
                - None / unspecified: interview every agent on both platforms
        """
        # Group by platform.
        twitter_interviews = []
        reddit_interviews = []
        both_platforms_interviews = []  # Items that need to run on both platforms.
        ...
        for interview in interviews:
            ...
            if item_platform == "twitter":
                ...
                # No platform specified: run on both platforms.
                both_platforms_interviews.append(interview)

        # Distribute both_platforms_interviews into each platform's list.
        ...
        # Process the Twitter interviews.
    ...
    def _get_interview_result(self, agent_id: int, platform: str) -> Dict[str, Any]:
        """Fetch the most recent Interview result from the database."""
    ...
    async def process_commands(self) -> bool:
        ...
        Returns:
            True to keep running, False to indicate the loop should exit.
        """
...
def get_agent_names_from_config(config: Dict[str, Any]) -> Dict[int, str]:
    ...
    This lets actions.jsonl show the real entity names instead of placeholders
    like "Agent_0".
...
def fetch_new_actions_from_db(
    ...
    Args:
        ...
        last_rowid: the largest rowid seen in the previous read
            (using rowid instead of created_at because created_at's format
    ...
    Returns:
        ...
        - actions_list: list of actions, each containing agent_id, agent_name,
          action_type, and action_args (with contextual info)
        - new_last_rowid: the new maximum rowid
    """
    actions = []
    ...
    try:
        ...
        # Track processed records by rowid (rowid is SQLite's built-in
        # auto-increment column). This avoids the created_at format mismatch
        # between platforms (Twitter uses integers, Reddit uses datetime strings).
        cursor.execute("""
        ...
        for rowid, user_id, action, info_json in cursor.fetchall():
            ...
            # Trim action_args down to the key fields (keep full content, do not truncate).
            simplified_args = {}
...
def _enrich_action_context(
    ...
    """
    Enrich an action with contextual info (post content, usernames, etc.).
    ...
    """
    try:
        # Like/dislike post: attach post content and author.
        ...
        # Repost: attach the original post's content and author.
        ...
        # Quote post: attach the original content, author, and quote comment.
            ...
            # Fetch the quote's own comment text (quote_content).
            if new_post_id:
                cursor.execute("""
        ...
        # Follow: attach the followed user's name.
        ...
        # Mute: attach the muted user's name.
        ...
        # Like/dislike comment: attach comment content and author.
        ...
        # Create comment: attach info about the post being commented on.
        ...
        # Failing to enrich context does not affect the main flow.
    ...
    try:
        ...
        if row:
            ...
            if agent_id is not None and agent_id in agent_names:
                ...
                # Otherwise look up the name in the user table.
...
def create_model(config: Dict[str, Any], use_boost: bool = False):
    ...
    When a boost LLM is configured, the parallel simulation can route each
    platform through a different API provider to increase concurrency.
    ...
    # Decide which LLM to use based on flag and configuration.
    ...
    # Fall back to config's model name if .env did not provide one.
    if not llm_model:
        llm_model = config.get("llm_model", "gpt-4o-mini")

    # Set the environment variables expected by camel-ai.
    if llm_api_key:
...
class PlatformSimulation:
    """Container for a platform simulation's result."""
    def __init__(self):
        self.env = None
...
async def run_twitter_simulation(
    ...
    Args:
        ...
        max_rounds: max number of simulation rounds (optional, used to truncate long runs)
    ...
    # Get the real agent name mapping from the config (using entity_name instead of the default Agent_X).
    agent_names = get_agent_names_from_config(config)
    # Fall back to OASIS's default name for any agent not present in config.
    ...
    total_actions = 0
    last_rowid = 0  # Track the last processed DB row (rowid avoids created_at format differences).

    # Execute the initial events.
    event_config = config.get("event_config", {})
    initial_posts = event_config.get("initial_posts", [])

    # Log the start of round 0 (initial-events phase).
    if action_logger:
    ...
    # If a max-rounds limit is set, truncate.
    ...
    for round_num in range(total_rounds):
        ...
        # Always log round start, regardless of whether any agents are active.
        if action_logger:
            action_logger.log_round_start(round_num + 1, simulated_hour)

        if not active_agents:
            # No active agents: still log round end (actions_count=0).
            if action_logger:
        ...
        # Pull actually-executed actions from the database and log them.
    ...
    # Note: do not close the environment; it is kept alive for Interviews.
    ...
    log_info(f"Simulation loop finished! Elapsed: {elapsed:.1f}s, total actions: {total_actions}")
...
async def run_reddit_simulation(
    ...
    # Reddit uses the boost LLM configuration (if available; otherwise falls back to primary).
...
async def main():
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help='Max simulation rounds (optional, used to truncate long runs)'
    )
    parser.add_argument(
        '--no-wait',
        action='store_true',
        default=False,
        help='Close the environment immediately after the simulation finishes (do not enter command-wait mode)'
    )
    ...
    log_manager.info(f"  - Total rounds from config: {config_total_rounds}")
    if args.max_rounds:
    ...
    # Holds the per-platform simulation results.
    ...
    if args.twitter_only:
        ...
        # Run in parallel (each platform gets its own logger).
    ...
    log_manager.info("=" * 60)
    log_manager.info(f"Simulation loop finished! Total elapsed: {total_elapsed:.1f}s")

    # Enter command-wait mode if requested.
    if wait_for_commands:
        log_manager.info("")
        log_manager.info("=" * 60)
        log_manager.info("Entering command-wait mode - environments stay alive")
...
def setup_signal_handlers(loop=None):
    """
    Install signal handlers so SIGTERM/SIGINT triggers a clean exit.

    This is a persistent-simulation scenario: the simulation does not exit when
    finished but waits for interview commands. On a termination signal we need to:
    1. Notify the asyncio loop to stop waiting.
    2. Give the program a chance to clean up (close databases, environments, etc.).
    3. Then exit.
    """
    def signal_handler(signum, frame):
        ...
        if not _cleanup_done:
            _cleanup_done = True
            # Set the event so the asyncio loop can exit (and clean up).
            if _shutdown_event:
                _shutdown_event.set()

        # Do not call sys.exit() directly; let the asyncio loop exit normally and release resources.
        # Only force exit if the same signal is received a second time.
        else:
            print("Force exiting...")
            sys.exit(1)
...
if __name__ == "__main__":
    ...
    try:
        ...
        # Clean up multiprocessing resource tracker (suppresses shutdown warnings).
        try:
